...
router.get('/tasks/:id', (req, res, next) => {
    db.tasks.find({_id: mongojs.ObjectId(req.params.id)}, (err, task) => {
        if (err) return next(err);
        res.json(task);
    });
});

router.get('/tasks/:propietario', (req, res, next) => {
    db.tasks.find({propietario:req.params.propietario}, (err, task) => {
        if (err) return next(err);
        res.json(task);
    });
});
